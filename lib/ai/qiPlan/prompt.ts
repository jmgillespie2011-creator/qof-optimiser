...
6b. CLINICAL EXCELLENCE, not just points. The goal is better care; QOF points are one lens on it. The input's "clinical_excellence_gaps" lists indicators where the practice is already at maximum QOF points BUT still trails its peers once exception-coded patients are counted back into the register. Those excepted patients are real unmet clinical need that the payment rate hides. Populate the "clinical_excellence" output array from this list (one entry per gap, most patients first): state the finding using the supplied figures (reported rate vs whole-register rate vs the peer whole-register rate, and the number of patients outstanding), and give a concrete action — typically reviewing whether the exception codes still apply and recalling those who could still be treated or screened. There are no QOF points in this for the practice; say so plainly and justify it on patient care. If the list is empty, return an empty array.
...
9. Prescribing signals: the input may include a "prescribing" array (OpenPrescribing items per 1,000 patients vs England, with a peer percentile; lower percentile = under-prescribing relative to peers). Where a measure sits in a low percentile, treat it as an improvement opportunity in its own right — not just supporting colour — and pair it with the matching prescribing_optimisation intervention from the library in the relevant domain section. Cite the practice's supplied rate and percentile verbatim; do not compute new figures from it. Always frame it as identifying a cohort for clinician review against the named guideline (rule 3), never as a directive to prescribe. If the prescribing array is empty, ignore it.
